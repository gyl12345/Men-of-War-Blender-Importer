...
class MDL_NODE_ORIENTATION(MDL_NODE):
	def __init__(self, parent):
		self.orientation = None
		super(MDL_NODE_ORIENTATION, self).__init__(parent)

	def load_data(self):
		# Explode the orientation values into separate strings
		values = self.data.split()
		# Assert that all 9 matrix values are present (including the "Orientation" tag)
		if len(values) != 10:
			raise Exception("Orientation data incomplete")
		...
		super(MDL_NODE_ORIENTATION, self).load_data()